...
def preserve_case(original: str, suggestion: str):
    if not suggestion: return suggestion
    if original.isupper(): return suggestion.upper()
    if original[:1].isupper(): return suggestion[:1].upper() + suggestion[1:]
    return suggestion

def add_issue(issues, issue_type, original, suggestion, explanation, rule):
    if not original or not suggestion or original.lower() == suggestion.lower(): return
    key = (issue_type, original.lower(), suggestion.lower())
    if any((i["type"], i["original"].lower(), i["suggestion"].lower()) == key for i in issues): return
...
def apply_context_rules(text: str, issues):
    ...
    for pattern, make_suggestion, explanation in patterns:
        for match in list(re.finditer(pattern, corrected, re.IGNORECASE)):
            ...
            if original.lower() == suggestion.lower(): continue
