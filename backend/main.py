...
def analyze_english(text: str):
    ...
    past_patterns = [
        ...
        (r"\b(he|she|it) eat\b(?=\s+(?:yesterday|last\b|ago\b))", "ate", "Use the past tense 'ate' with a past-time expression."),
        (r"\b(he|she|it) see\b(?=\s+(?:yesterday|last\b|ago\b))", "saw", "Use the past tense 'saw' with a past-time expression."),
        (r"\b(he|she|it) come\b(?=\s+(?:yesterday|last\b|ago\b))", "came", "Use the past tense 'came' with a past-time expression."),
    ]
    ...
    for pattern, replacement, explanation in past_patterns:
        ...
        if match:
            ...
            past_matches.add(original.lower())
            issues.append({
                "type": "grammar",
                "original": original,
    ...
    for pattern, replacement, explanation in grammar_rules:
        ...
        if not match:
            continue
        wrong = match.group(0)
        if wrong.lower() in past_matches:
            continue
        corrected = re.sub(
            r"\b" + re.escape(wrong.split()[-1]) + r"\b",
            replacement,
            wrong,
            count=1,
            flags=re.IGNORECASE,
